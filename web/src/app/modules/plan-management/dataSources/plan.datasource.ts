...
export class PlansDataSource extends CommonDataSource<Plan> {
  ...
  protected mapToDataRows(plans: Plan[]): DataRow[] {
    return plans.map((plan) => ({
      ...
      [COLUMN_KEYS.TITLE]: Cell.createStringCell(plan.title),
      [COLUMN_KEYS.STAGE]: Cell.createStringCell(plan.activeStage?.keyString),
      [COLUMN_KEYS.CREATOR]: Cell.createLinkCell(plan.Creator?.fullname, UrlGenerator.getUserUrl(plan.Creator?.id)),
      [COLUMN_KEYS.PARTICIPANTS]: Cell.createStringCell(plan.Participants?.length),
      [COLUMN_KEYS.CREATED_AT]: Cell.createDateCell(plan.createdAt),
      [COLUMN_KEYS.UPDATED_AT]: Cell.createDateCell(plan.updatedAt),
