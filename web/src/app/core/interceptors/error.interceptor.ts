...
import { API_ROUTES, PATHS, ROUTING } from '@/shared/constants';
...
const genericRetryStrategy = ({
  ...
  excludedStatusCodes = [],
  excludeUrl = [API_ROUTES.REFRESH_SESSION]
}: {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    return next.handle(request).pipe(
      catchError((err) => {
        ...
        if (err.status >= 500) {
          this.router.navigateByUrl(ROUTING.ROUTE_INTERNAL_ERROR);
        } else if (err.status === 401 && !request.url.includes(API_ROUTES.REFRESH_SESSION)) {
          this.store$.dispatch(refreshSession());
        } else if (err.status === 403 && request.url.includes(API_ROUTES.REFRESH_SESSION)) {
