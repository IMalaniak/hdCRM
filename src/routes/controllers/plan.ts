...
export class PlanController {
    ...
    private get(req: Request, res: Response) {
        ...
        this.findPlanById(req.params.id).then((plan: db.Plan) => {
                ...
                plan.setCreator(user).then(() => {
                    db.Stage.findAll({
                        ...
                        for (const i in stages) {
                            ...
                            plan.addStage(stages[i], {through: {
                                    order: i,
                                    completed: false
                                }
                            });
                ...
                db.Plan.findByPk(req.body.id).then(plan => {
                    if (req.body.Participants) {
                        db.User.findAll({
                            ...
                            plan.setParticipants(users).then(() => {
                                this.findPlanById(req.body.id).then(plan => {
                                    return res.status(OK).json(plan);
                                }).catch((err: any) => {
                                    Logger.Err(err);
                                    return res.status(BAD_REQUEST).json(err);
                    ...
                    stage.Details.save().then(stage => {
                        if (plan.Stages[i + 1]) {
                            plan.setActiveStage(plan.Stages[i + 1].id).then(() => {
                                this.findPlanById(req.params.id).then(plan => {
                                    return res.status(OK).json(plan);
                                }).catch((error: any) => {
                                    Logger.Err(error);
                                    return res.status(BAD_REQUEST).json(error.toString());
