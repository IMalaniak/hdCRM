...
export const selectUsersByState = (stateId: number) => createSelector(
    selectAllUsers,
    users => users.filter(user => user.StateId === stateId)
);

export const allUsersLoaded = createSelector(
  selectUsersState,
  userState => userState.allUsersLoaded
);
...
export const selectUsersPagesCount = createSelector(
  selectUsersState,
  usersState => usersState.pages
);

export const selectUsersTotalCount = createSelector(
  selectUsersState,
  usersState => usersState.countAll
);

export const selectUsersPage = (page: PageQuery) => createSelector(
  selectAllUsers,
  selectUsersPagesCount,
  (allUsers: User[], pagesCount: number) => {
    if (!pagesCount) {
      return [];
    } else {
...
export const allStatesLoaded = createSelector(
  selectStatesState,
  stateState => stateState.allStatesLoaded
);