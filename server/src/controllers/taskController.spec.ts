...
describe('TaskController', () => {
  ...
  it('should send success response when calling getAll', async () => {
    ...
    getAllStub.resolves(ok({ success: true, data: [taskFake] }));
    ...
    expect(resFake.send.calledOnceWith({ success: true, data: [taskFake] })).to.be.true;
    getAllStub.restore();
  });
  ...
  it('should send success response when calling create', async () => {
    ...
    createStub.resolves(ok({ success: true, data: taskFake }));
    ...
    expect(resFake.status.calledOnceWith(StatusCodes.OK)).to.be.true;
    expect(resFake.send.calledOnceWithExactly({ success: true, data: taskFake })).to.be.true;
    createStub.restore();
  });
  ...
  it('should send success response when calling delete', async () => {
    ...
    deleteStub.resolves(ok({ success: true, message: `Deleted 2` }));
    ...
    expect(resFake.status.calledOnceWith(StatusCodes.OK)).to.be.true;
    expect(resFake.send.calledOnceWithExactly({ success: true, message: `Deleted 2` })).to.be.true;
    deleteStub.restore();
  });
